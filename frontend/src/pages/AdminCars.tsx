...
import { Loader2, Plus, X, Car as CarIcon, ArrowLeft, Edit3, Trash2, Upload } from 'lucide-react';
...
const BrandLogo = ({ name, url }: { name: string, url?: string }) => {
    ...
    if (!url || error) {
        return (
            <div className="w-8 h-8 rounded-full bg-white/10 flex items-center justify-center text-xs font-bold text-white uppercase flex-shrink-0 border border-white/10">
                {name.substring(0, 2)}
            </div>
        );
    }
            ...
            <div className="container mx-auto max-w-7xl space-y-8">
                        ...
                        <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            <div>
                                ...
                                <select required className={inputClass} value={formData.category} onChange={e => setFormData({ ...formData, category: e.target.value as any })}>
                                    <option value="ECONOMY" className="bg-dark-bg">Ekonomi</option>
                                    <option value="COMPACT" className="bg-dark-bg">Kompakt</option>
                                    <option value="MIDSIZE" className="bg-dark-bg">Orta</option>
                                    <option value="FULLSIZE" className="bg-dark-bg">Büyük</option>
                            ...
                            <div>
                                ...
                                <input type="number" required min="0" step="0.01" className={inputClass} value={formData.dailyPrice} onChange={e => setFormData({ ...formData, dailyPrice: Number(e.target.value) })} />
                            </div>
                            <div>
                            ...
                            <div>
                                ...
                                <select required className={inputClass} value={formData.branchId} onChange={e => setFormData({ ...formData, branchId: e.target.value })}>
                                    {branches.map(b => (
                            ...
                            <div className="lg:col-span-3">
                                ...
                                {formData.images.length > 0 && (
                                    <div className="mt-6">
                                        <label className={labelClass}>Yüklenen Fotoğraflar ({formData.images.length})</label>
                ...
                <div className="bg-dark-surface-lighter/80 backdrop-blur-xl rounded-2xl border border-white/10 overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.3)]">
                    <div className="p-6 border-b border-white/10 flex justify-between items-center">
                        ...
                        <span className="text-xs font-bold text-gray-400 bg-dark-bg px-3 py-1.5 rounded-full border border-white/5">{cars.length} araç</span>
                    </div>
                                ...
                                {cars.length === 0 ? (
                                    <tr>
                                        <td colSpan={7} className="p-12 text-center">
                                            <CarIcon className="w-12 h-12 mx-auto mb-3 text-gray-600" />
                                            <p className="text-gray-400">Henüz araç eklenmemiş</p>
                                        </td>
                                    </tr>
                                ) : (
                                    cars.map(car => (
                                        <tr key={car.id} className="hover:bg-white/5 transition-colors">
                                            ...
                                            <td className="p-4 text-gray-300">{car.category}</td>
                                            <td className="p-4 text-sm text-gray-400">
                                                {car.transmission === 'AUTO' ? 'Otomatik' : 'Manuel'} / {car.fuel}
                                            </td>
